...
def draw_trend(timeSeries,size0,size1,title):
    ...
    # 对size个数据进行加权移动平均
    rol_weighted_mean = timeSeries.ewm(span=size0,min_periods=0,adjust=True,ignore_na=False).mean().ewm(span=size1,min_periods=0,adjust=True,ignore_na=False).mean();
...
bound=50000;
#单笔大于或小于1W的购买赎回分析
user_balance_op_more_than_bound=user_balance_table[(user_balance_table['total_purchase_amt'] >= bound) | (user_balance_table['total_redeem_amt'] >= bound)];
...
purchase_redeem_total_more_than_bound=purchase_redeem_total_more_than_bound.rename(columns={'total_purchase_amt' : 'total_purchase_amt_more_than_bound'});
purchase_redeem_total_more_than_bound=purchase_redeem_total_more_than_bound.rename(columns={'total_redeem_amt' : 'total_redeem_amt_more_than_bound'});
#purchase_redeem_total_more_than_bound.plot();


user_balance_op_less_than_bound=user_balance_table[(user_balance_table['total_purchase_amt'] < bound) & (user_balance_table['total_redeem_amt'] < bound)];
...
purchase_redeem_total_less_than_bound=purchase_redeem_total_less_than_bound.rename(columns={'total_purchase_amt' : 'total_purchase_amt_less_than_bound'});
purchase_redeem_total_less_than_bound=purchase_redeem_total_less_than_bound.rename(columns={'total_redeem_amt' : 'total_redeem_amt_less_than_bound'});
...
purchase_redeem_total_concat_result = pd.concat([purchase_redeem_total_concat_result, purchase_redeem_total_less_than_bound], axis=1);
...
draw_trend(purchase_redeem_total['total_purchase_amt']-purchase_redeem_total['total_redeem_amt'],7,30,'purchase_cut_redeem_Rolling Mean ');
plt.draw()
...
draw_PSD((purchase_redeem_total['total_purchase_amt']-purchase_redeem_total['total_redeem_amt']).tolist(),1,"total_purchase_cut_redeem_amt Power Spectrum")
plt.draw()
#收益率绘制
...
offset_corr(corr_pd['total_purchase_amt'], corr_pd['mfd_7daily_yield'],90,200,"total_purchase mfd_7daily_yield corr");
...
offset_corr(corr_pd['total_redeem_amt'], corr_pd['mfd_7daily_yield'],90,200,"total_redeem_amt mfd_7daily_yield corr");
offset_corr(corr_pd['total_redeem_amt'], corr_pd['mfd_7daily_yield'],120,140,"total_redeem_amt mfd_7daily_yield corr 120d offset");
...
corr_table.to_csv('./corr_head_rm.csv',index=True,sep=',');

#去除尾部90多个数据的相关性分析
corr_tail_rm_table=corr_pd.head(300).corr();
print(corr_tail_rm_table);
corr_table.to_csv('./corr_tail_rm.csv',index=True,sep=',');

#去除首尾部90多个数据的相关性分析
corr_head_tail_rm_table=corr_pd.head(300).tail(200).corr();
print(corr_head_tail_rm_table);
corr_table.to_csv('./corr_head_tail_rm.csv',index=True,sep=',');
